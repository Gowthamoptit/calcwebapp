pipeline {
    ...
    stages {
        ...
        stage('tomcat') {
            steps {
                deploy adapters: [tomcat9(credentialsId: '0772643b-f884-412e-a9e3-63eb2567206b', path: '', url: '"{tomcat_url}"')], contextPath: null, war: 'target/*.war', parameters: [string(name: 'tomcat_url', value: "${params.tomcat_url}")]
            }
        }
        stage('tomcat1') {
            steps {
